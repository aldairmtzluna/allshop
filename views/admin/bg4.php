...
<div class="container-bg">
    ...
    if (isset($archivo) && $archivo != "") {
        ...
        if (!((strpos($tipo, "gif") || strpos($tipo, "jpeg") || strpos($tipo, "jpg") || strpos($tipo, "png")) && ($tamano < 2000000))) {
            ...
            if (move_uploaded_file($temp, '../img/bg/'.$archivo)) {
                $queryImg = $conn->query ("UPDATE sliders SET fondo4_slide ='$nameF'");
    ...
    ?>
    <?php
        $query = $conn->query("SELECT fondo4_slide FROM sliders");
        if ($fila = mysqli_fetch_array($query)) {
    ?>
        <div class="panel-bg">
                <img src="<?php echo SERVERURL ?>views/img/bg/<?php echo $fila ['fondo4_slide'];?>">
        </div>
    <?php
        }
    ?>
